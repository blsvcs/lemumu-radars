 ...
 try:
  ...
  if raw.startswith(b'%PDF'):
   ...
   with zipfile.ZipFile(io.BytesIO(raw)) as z:
    parts=[n for n in z.namelist() if n=='word/document.xml' or n=='content.xml']
    if 'xl/workbook.xml' in z.namelist():
     ns={'x':'http://schemas.openxmlformats.org/spreadsheetml/2006/main'}
     shared=[]
     if 'xl/sharedStrings.xml' in z.namelist(): shared=[''.join(e.itertext()) for e in ElementTree.fromstring(z.read('xl/sharedStrings.xml'))]
     lines=[]
     for name in z.namelist():
      if not re.fullmatch(r'xl/worksheets/sheet[0-9]+.xml',name):continue
      if z.getinfo(name).file_size>15_000_000:raise ValueError('Pārāk liela Excel lapa')
      for row in ElementTree.fromstring(z.read(name)).findall('.//x:row',ns):
       cells=[]
       for c in row.findall('x:c',ns):
        ...
        if formula is not None:value=(value or 'NAV SAGLABĀTAS VĒRTĪBAS')+' [formula: '+(formula.text or '')+']'
        if value:cells.append(c.attrib.get('r','')+': '+value)
       if cells:lines.append(name+' | '+' | '.join(cells))
     text='\n'.join(lines)
     parts=[]
    if not parts and 'xl/workbook.xml' not in z.namelist():raise ValueError('Arhīva formāts nav atbalstīts; nepieciešama atsevišķa pārbaude')
    if any(z.getinfo(n).file_size>15_000_000 for n in parts):raise ValueError('Pārāk liels izpakots dokuments')
    if parts:text='\n'.join(' '.join(ElementTree.fromstring(z.read(n)).itertext()) for n in parts)
  ...
  result={**entry,'checked':now,'status':'read','text':text,'hash':newhash,'history':history[-50:],'links':child}
...
def main():
 ...
 for idx,row in enumerate(rows):
  ...
  try:future=datetime.strptime(date[:10],'%d.%m.%Y').date()>=datetime.now(timezone.utc).date()
 ...
 for row,date in selected:
  try:
   ...
   for item in m['items']:
    ...
    compare=['title','status','presenter','position','documents','participation']
    if prior and not olditem:changes.append({'at':now,'kind':'Pievienots novērotajai darba kārtībai','id':item['id'],'meeting':m['url']})
    elif olditem:
     changed=[k for k in compare if olditem.get(k)!=item.get(k)]
 ...
 entries={d['url']:d for m in meetings[:len(selected)] for i in m['items'] if i['id'].split()[0] in targets for d in i['documents']}
 for m in meetings[:len(selected)]:
  for d in m['protocols']:entries[d['url']]=d
 documents=[]
 with ThreadPoolExecutor(max_workers=3) as pool:
  documents=list(pool.map(document,entries.values()))
 # Inspect discovered public attachments and nested document links, with an explicit cap.
 seen=set(entries);frontier={l['url']:l for d in documents for l in d.get('links',[]) if l['url'] not in seen}
 for depth in range(3):
  if not frontier:break
  batch=list(frontier.values())[:150];seen.update(x['url'] for x in batch)
  with ThreadPoolExecutor(max_workers=3) as pool:new=list(pool.map(document,batch))
  documents.extend(new);frontier={l['url']:l for d in new for l in d.get('links',[]) if l['url'] not in seen}
 if frontier:errors.append({'error':'Palikušas neiegūtas saites pēc automātiskās dziļuma vai apjoma robežas; analīzē jāatzīmē nepilnīgs pārklājums.'})
 for d in documents:d.pop('links',None)
 path.write_text(json.dumps({'checked':now,'baseline':not bool(previous),'scope':'Jaunākās trīs un visas sarakstā redzamās gaidāmās MK sēdes. Agrākie momentuzņēmumi saglabāti. Pilns saistīto dokumentu iegūšanas mēģinājums redakcijas izvēlētajiem projektiem; kļūmes norādītas atsevišķi.','meetings':meetings,'changes':changes[-300:],'documents':documents,'errors':errors},ensure_ascii=False,indent=2)+'\n')
